...
//
// Windows CUDA CI uses clang + MSVC ABI (link.exe via clang driver; MinGW
// cannot link the CUDA toolkit). DLL import libs are referenced via -l so the
// clang driver resolves them through /LIBPATH instead of requiring literal
// file paths. Runtime needs cudart/cublas/cublasLt DLLs next to the binary.
//
...
#cgo windows LDFLAGS: ${SRCDIR}/../../third_party/llama.cpp/build/src/llama.lib ${SRCDIR}/../../third_party/llama.cpp/build/ggml/src/ggml.lib ${SRCDIR}/../../third_party/llama.cpp/build/ggml/src/ggml-base.lib ${SRCDIR}/../../third_party/llama.cpp/build/ggml/src/ggml-cpu.lib ${SRCDIR}/../../third_party/llama.cpp/build/ggml/src/ggml-cuda/ggml-cuda.lib -lcudart -lcublas -lcublasLt -lcuda -ladvapi32
