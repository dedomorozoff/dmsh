...
func (m *tuiModel) showHelp() {
	...
	m.addLine("dmsh is a natural language shell. Type \"show files\" and it runs \"ls -la\" for you.")
	m.addLine("")
	m.addLine(fmt.Sprintf("%sModes:%s", colorBold, colorReset))
	m.addLine(fmt.Sprintf("  %sAI%s    — AI generates and executes commands automatically (default)", colorYellow, colorReset))
	m.addLine(fmt.Sprintf("  %sHelp%s  — AI shows command + explanation, you run it manually", colorYellow, colorReset))
	m.addLine(fmt.Sprintf("  %sShell%s — Direct shell command execution, NL requests via AI", colorYellow, colorReset))
	m.addLine("")
	m.addLine(fmt.Sprintf("%sCommands:%s", colorBold, colorReset))
	m.addLine("  plain text    — send request to LLM")
	m.addLine(fmt.Sprintf("  %s!command%s   — execute command directly", colorYellow, colorReset))
	m.addLine(fmt.Sprintf("  %s/cd%s path   — change directory", colorYellow, colorReset))
	m.addLine(fmt.Sprintf("  %s/clear%s     — clear screen", colorYellow, colorReset))
	m.addLine(fmt.Sprintf("  %s/pwd%s       — show current directory", colorYellow, colorReset))
	m.addLine(fmt.Sprintf("  %s/history%s   — show history", colorYellow, colorReset))
	m.addLine(fmt.Sprintf("  %s/mode%s      — show current mode", colorYellow, colorReset))
	m.addLine(fmt.Sprintf("  %s/mode ai%s   or %s/mode 1%s or %s/1%s — AI mode", colorYellow, colorReset, colorYellow, colorReset, colorYellow, colorReset))
	m.addLine(fmt.Sprintf("  %s/mode help%s or %s/mode 2%s or %s/2%s — Help mode", colorYellow, colorReset, colorYellow, colorReset, colorYellow, colorReset))
	m.addLine(fmt.Sprintf("  %s/mode shell%s or %s/mode 3%s or %s/3%s — Shell mode", colorYellow, colorReset, colorYellow, colorReset, colorYellow, colorReset))
	m.addLine(fmt.Sprintf("  %s/stats%s     — session statistics", colorYellow, colorReset))
	m.addLine(fmt.Sprintf("  %s/model%s     — show the model currently in use", colorYellow, colorReset))
	m.addLine(fmt.Sprintf("  %s/retry%s     — re-run last request with alternate approach", colorYellow, colorReset))
	m.addLine(fmt.Sprintf("  %s/export%s    — copy last command to clipboard or /export last > file", colorYellow, colorReset))
	m.addLine(fmt.Sprintf("  %s/alias%s     — list aliases; /alias name=\"request\" to create; /alias -d name to delete", colorYellow, colorReset))
	m.addLine(fmt.Sprintf("  %s/bind%s      — show key bindings", colorYellow, colorReset))
	m.addLine(fmt.Sprintf("  %s/exit%s      — exit", colorYellow, colorReset))
	m.addLine("")
	m.addLine(fmt.Sprintf("%sKeybindings:%s", colorBold, colorReset))
	m.addLine(fmt.Sprintf("  %sCtrl+A%s     — start of line     %sCtrl+E%s     — end of line", colorYellow, colorReset, colorYellow, colorReset))
	m.addLine(fmt.Sprintf("  %sCtrl+R%s     — history search    %sCtrl+S%s     — forward search", colorYellow, colorReset, colorYellow, colorReset))
	m.addLine(fmt.Sprintf("  %sCtrl+P%s     — previous          %sCtrl+N%s     — next", colorYellow, colorReset, colorYellow, colorReset))
	m.addLine(fmt.Sprintf("  %sCtrl+U%s     — delete to start   %sCtrl+K%s     — delete to end", colorYellow, colorReset, colorYellow, colorReset))
	m.addLine(fmt.Sprintf("  %sAlt+B%s      — back one word     %sAlt+F%s      — forward one word", colorYellow, colorReset, colorYellow, colorReset))
	m.addLine(fmt.Sprintf("  %sCtrl+W%s     — delete word back  %sAlt+D%s    — delete word forward", colorYellow, colorReset, colorYellow, colorReset))
	m.addLine(fmt.Sprintf("  %sCtrl+L%s     — clear screen      %s/exit%s      — exit", colorYellow, colorReset, colorYellow, colorReset))
	m.addLine("")
	m.addLine(fmt.Sprintf("%sExamples:%s  show all txt files, find errors in logs, start docker", colorBold, colorReset))
	m.addLine(fmt.Sprintf("%s Default: %sdry-run=false%s (commands execute). Use --dry-run to enable safe mode.%s", colorBold, colorGreen, colorReset, colorReset))
}

func (m *tuiModel) showKeyBindings() {
	m.addLine(fmt.Sprintf("%s=== Available Keybindings ===%s", colorBold+colorCyan, colorReset))
	m.addLine(fmt.Sprintf("%sBasic:%s", colorBold, colorReset))
	m.addLine(fmt.Sprintf("  %sCtrl+A%s     — beginning of line", colorYellow, colorReset))
	m.addLine(fmt.Sprintf("  %sCtrl+E%s     — end of line", colorYellow, colorReset))
	m.addLine(fmt.Sprintf("  %sCtrl+U%s     — delete to beginning of line", colorYellow, colorReset))
	m.addLine(fmt.Sprintf("  %sCtrl+K%s     — delete to end of line", colorYellow, colorReset))
	m.addLine(fmt.Sprintf("  %sCtrl+L%s     — clear screen", colorYellow, colorReset))
	m.addLine(fmt.Sprintf("  %sCtrl+R%s     — reverse history search", colorYellow, colorReset))
	m.addLine(fmt.Sprintf("  %sCtrl+S%s     — forward history search", colorYellow, colorReset))
	m.addLine(fmt.Sprintf("  %sCtrl+P%s     — previous command", colorYellow, colorReset))
	m.addLine(fmt.Sprintf("  %sCtrl+N%s     — next command", colorYellow, colorReset))
	m.addLine(fmt.Sprintf("  %sAlt+B%s      — back one word", colorYellow, colorReset))
	m.addLine(fmt.Sprintf("  %sAlt+F%s      — forward one word", colorYellow, colorReset))
	m.addLine(fmt.Sprintf("  %sAlt+D%s      — delete forward one word", colorYellow, colorReset))
	m.addLine(fmt.Sprintf("  %sCtrl+W%s     — delete backward one word", colorYellow, colorReset))
	m.addLine(fmt.Sprintf("  %sCtrl+D%s     — exit (delete char if line not empty)", colorYellow, colorReset))
	m.addLine(fmt.Sprintf("  %sCtrl+O%s     — model menu (install / switch on the fly)", colorYellow, colorReset))
	m.addLine(fmt.Sprintf("  %sTab%s        — complete slash command", colorYellow, colorReset))
	m.addLine(fmt.Sprintf("  %s↑/↓%s        — choose from the / menu, %sEnter%s — run it", colorYellow, colorReset, colorYellow, colorReset))
	m.addLine("")
	m.addLine(fmt.Sprintf("%sModes (shortcuts):%s", colorBold, colorReset))
	m.addLine(fmt.Sprintf("  %s/1%s or %s/mode 1%s            — AI mode (auto-execute)", colorYellow, colorReset, colorYellow, colorReset))
	m.addLine(fmt.Sprintf("  %s/2%s or %s/mode 2%s            — Help mode (command + explanation)", colorYellow, colorReset, colorYellow, colorReset))
	m.addLine(fmt.Sprintf("  %s/3%s or %s/mode 3%s            — Shell mode (direct execution)", colorYellow, colorReset, colorYellow, colorReset))
	m.addLine("")
	m.addLine(fmt.Sprintf("%sSpecial:%s", colorBold, colorReset))
	m.addLine(fmt.Sprintf("  %s/exit%s      — exit REPL", colorYellow, colorReset))
	m.addLine(fmt.Sprintf("  %s/cd%s path   — change directory", colorYellow, colorReset))
	m.addLine(fmt.Sprintf("  %s/clear%s     — clear screen", colorYellow, colorReset))
	m.addLine(fmt.Sprintf("  %s/pwd%s       — show current directory", colorYellow, colorReset))
	m.addLine(fmt.Sprintf("  %s/history%s   — show history", colorYellow, colorReset))
	m.addLine(fmt.Sprintf("  %s/mode%s      — show current mode", colorYellow, colorReset))
	m.addLine(fmt.Sprintf("  %s/bind%s      — show this list", colorYellow, colorReset))
	m.addLine(fmt.Sprintf("  %s!command%s   — execute command directly", colorYellow, colorReset))
	m.addLine("")
	m.addLine(fmt.Sprintf("%sCompletion:%s", colorBold, colorReset))
	m.addLine(fmt.Sprintf("  %sTab%s        — auto-complete slash commands", colorYellow, colorReset))
}
